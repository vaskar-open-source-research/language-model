...
def optmized_train_bpe(input_path: str, vocab_size: int, special_tokens: List[str]):
    ...
    for k, (token_bytes, value) in tqdm(enumerate(list_of_pre_token_bytes_counter)):
        for i in range(1, len(token_bytes)):
            byte_pair = (token_bytes[i-1], token_bytes[i])
            if byte_pair not in byte_pair_counts:
                byte_pair_counts[byte_pair] = 0
    ...
    with tqdm(total=vocab_size - len(vocab), desc="Train") as pbar:
        while pre_token_bytes and len(vocab) < vocab_size:
            pbar.update(1)
